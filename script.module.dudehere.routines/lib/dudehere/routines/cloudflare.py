...
def solve(url, cookie_file, user_agent, wait=True):
	...
	if not init_match or not vc_match or not pass_match:
		ADDON.log("Couldn't find attribute: init: |%s| vc: |%s| pass: |%s| No cloudflare check?" % (init_match, vc_match, pass_match))
		return False
	...
	ADDON.log('Initial value: |%s| Result: |%s|' % (init_equation, result))
	
	for equation in equations.split(';'):
			equation = equation.rstrip()
			if equation[:len('.'.join(varname))] != '.'.join(varname):
				ADDON.log('Equation does not start with varname |%s|' % (equation))
			else:
			...
			operator = equation[0]
			if operator not in ['+', '-', '*', '/']:
				ADDON.log('Unknown operator: |%s|' % (equation))
				continue
				
			result = int(str(eval(str(result) + operator + str(solve_equation(expression)))))
			ADDON.log('intermediate: %s = %s' % (equation, result))
	
	scheme = urlparse.urlparse(url).scheme
	domain = urlparse.urlparse(url).hostname
	result += len(domain)
	ADDON.log('Final Result: |%s|' % (result))

	if wait:
		ADDON.log('Sleeping for 5 Seconds')
		xbmc.sleep(5000)
	...
	try:
		...
		ADDON.log('CloudFlare Error: %s on url: %s' % (e.code, url))
		return False

	if cj is not None:
		cj.save()
		ADDON.log('CF Save Cookies')
