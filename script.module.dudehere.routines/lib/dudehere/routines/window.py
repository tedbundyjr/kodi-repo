...
class Window(pyxbmct.AddonDialogWindow):
	def __init__(self, title='', width=None, height=None, columns=None, rows=None, shade=False, quiet=False):
	...
	def draw(self):
		...
		'''
		if self.shade:
			self.create_image('shading', VFSClass().join(TEXTURES, 'default/AddonWindow/shading.png'), aspectRatio=1)
	...
	def draw_progress_bar(self, name):
		...
		bg = xbmcgui.ControlImage(pos_x, pos_y, 15, 24, TEXTURES + '/OSDProgressBackLeft.png', aspectRatio=0)
		self.addControl(bg)
		self.__get_object(name)._highlight_images.append(xbmcgui.ControlImage(pos_x, pos_y, 15, 24, TEXTURES + '/OSDProgressMidLeft.png', aspectRatio=0))
		self.addControl(self.__get_object(name)._highlight_images[0])
		self.__get_object(name)._highlight_images[0].setVisible(False)
		bg = xbmcgui.ControlImage(pos_x + width - 15, pos_y, 15, 24, TEXTURES + '/OSDProgressBackRight.png', aspectRatio=0)
		self.addControl(bg)
		...
		for n in xrange(segments):
			n += 1
			offset_x = n * 10
			bg = xbmcgui.ControlImage(pos_x + 5 + offset_x, pos_y, 10, 24, TEXTURES + '/OSDProgressBack.png', aspectRatio=1)
			self.addControl(bg)
			
			self.__get_object(name)._highlight_images.append(xbmcgui.ControlImage(pos_x + 5 + offset_x, pos_y, 10, 24, TEXTURES + '/OSDProgressMid.png', aspectRatio=1))
		...
		bg = xbmcgui.ControlImage(pos_x + 15 + offset_x, pos_y, rest, 24, TEXTURES + '/OSDProgressBack.png', aspectRatio=1)
		self.addControl(bg)
		self.__get_object(name)._highlight_images.append( xbmcgui.ControlImage(pos_x + 15 + offset_x, pos_y, rest, 24, TEXTURES + '/OSDProgressMid.png', aspectRatio=1))
		n += 1
		self.addControl(self.__get_object(name)._highlight_images[n])
		self.__get_object(name)._highlight_images[n].setVisible(False)
		n += 1
		self.__get_object(name)._highlight_images.append( xbmcgui.ControlImage(pos_x + width - 15, pos_y, 15, 24, TEXTURES + '/OSDProgressMidRight.png', aspectRatio=0))
		self.addControl(self.__get_object(name)._highlight_images[n])
		self.__get_object(name)._highlight_images[n].setVisible(False)
	...
	def add_list_items(self, name, items, default=None, selectable=True, allow_multiple=False, allow_toggle=True, call_back=None):
		...
		#self.connectEventList([pyxbmct.ACTION_MOUSE_LEFT_CLICK, ACTION_ENTER, ACTION_SELECT_ITEM], self.__list_update)
		def list_update():
			if selectable:
				obj = self.getFocus()
				...
				if list_item.getLabel2() == "checked":
					...
					list_item.setIconImage(TEXTURES + 'checked.png')
	...
	def set_selected(self, key, index):
		if self.__get_object(key)._type == 'list':
			...
			self.__get_object(key).getListItem(index).setIconImage(TEXTURES + 'checked.png')
		else:
			pass
