...
#######################################
        ...
        def getPlEntryIcon(self,mediaitem):
            ...
            if URL != '':
                ...
                try: loader.load(URL,imageCacheDir+"icon."+ext,timeout=10,proxy="ENABLED",content_type='image')
                except: loader.state=0
                if loader.state==0:
        ...
        def getPlEntryThumb(self,mediaitem):
            ...
            if mediaitem.thumb != 'default':
                ...
                ext=getFileExtension(URL)
                loader=CFileLoader2() #file loader
                try: loader.load(URL,imageCacheDir+"thumb."+ext,timeout=2,proxy="INCACHE",content_type='image')
                except: loader.state=0
                if loader.state==0:
                    return loader.localfile
...
#                return
                ...
                if (playlist != 0) and (playlist.playmode=='autonext'):
                    ...
                    print 'line 1087naivx.py: '+str(mediaitem.URL); result=MyPlayer.play_URL(mediaitem.URL,mediaitem);
          ...
          except: pass; print '* Error during navix.py - SelectItem.'; self.state_busy=0
        ######################################################################
        ...
        def onPlayUsing(self):
            ...
            if (choice != -1) and (choice < 8): #if not cancel
                ...
                if result["code"]==1:
                    dialog=xbmcgui.Dialog()
                    dialog.ok("Error","Cannot open file."); print ["Error","1","Cannot open file.","navix.py - onPlayUsing",URL]
        ...
        def OpenTextFile(self,URL='',mediaitem=CMediaItem()):
          #try:
            ...
            if len(mediaitem.URL)==0:
              ...
              elif ('My Downloads' in URL) or ('My Playlists' in URL): SplashTxt=FileDoOpen(os.path.join(datapaths,mediaitem.URL)); print 'txt is a local file in a different path';
              ...
              elif ('My Downloads' in mediaitem.URL) or ('My Playlists' in mediaitem.URL): SplashTxt=FileDoOpen(os.path.join(datapaths,mediaitem.URL)); print 'txt is a local file in a different path';
              elif (':' in mediaitem.URL): SplashTxt=FileDoOpen(mediaitem.URL); print 'txt is a local file in a different path';print mediaitem.URL
        ...
        def InstallApp(self,URL='',mediaitem=CMediaItem()):
            ...
            if type=='script':
                ...
                if '?' in mediaitem.URL:
                    ...
                    try: xbmc.executebuiltin('RunAddon(%s,"%s")'%(URLa,URLb));
                    except: xbmc.executebuiltin('RunAddon(%s)'%URLa);
                else:
...
#            self.ParsePlaylist(reload=False) #display main list
        ...
        def selectBoxDownloadsList(self):
            if   self.URL==downloads_file:
                ...
                possibleChoices=["Play...","Download / Add to Queue","Download Queue","Download Queue and Shutdown","Stop Downloading","Remove Item","Cancel"]
                dialog=xbmcgui.Dialog()
                ...
                if choice==2 or choice==3: #Download Queue / Shutdown
                    ...
                    if dialog.yesno("Message","This will remove the entry and delete the", \
                        ...
                        if os.path.exists(mediaitem.DLloc):
                            try: os.remove(mediaitem.DLloc)
                            except Exception as e: print('navi.py line2208  '+str(e)); pass
        ...
        def selectBoxMainList(self,choice=-1):
            ...
            if   choice==0: #Download
                ...
                self.onClearHistory()
            elif choice==5: #Refresh Page From Server
                self.onRefreshPage(self.URL,self.mediaitem)
            elif choice==6: #Block selected playlist
                self.onParentalControl()
            elif choice==7: #Set default player
                self.onSetDefaultPlayer()
            elif choice==8: #Set Background
                self.onSetBackGround()
            elif choice==9: #Slideshow

                pos=self.getPlaylistPosition()
                self.viewImage(self.playlist,pos,1) #slide show show
            elif choice==10: #Add selected item to Favorites
                pos=self.getPlaylistPosition()
                tmp=CMediaItem() #create new item
                tmp.type=self.playlist.list[pos].type
            ...
            elif choice==11: #Add playlist to Favorites
                tmp=CMediaItem() #create new item
                ...
                self.addToFavorites(tmp)
            elif choice==12: # Create playlist shortcut
                self.CreateShortCut()
            elif choice==13: #Set Playlist as Home
                if dialog.yesno("Message","Overwrite current Home playlist?")==False:
                    return
                self.home=self.URL
 #               self.onSaveSettings()
            elif choice==14: #View playlist source
                self.pl_focus.save(RootDir+source_list)
                self.OpenTextFile(RootDir+source_list)
            elif choice==15: #Set smart caching on/off
                self.onSetSmartCaching()
            elif choice==16: #about Navi-X
                self.OpenTextFile('readme.txt')
            elif choice==17: #Exit Navi-X
                #self.setFocus(self.list3); xbmc.sleep(10); self.onAction1(ACTION_SELECT_ITEM)
                dialog=xbmcgui.Dialog()
                if dialog.yesno("Navi-X","Are you sure you want to leave?")==True:
                    self.state_busy=1
        ...
        def onReadSearchHistory(self):
            try:
                ...
                try: FileDoSave(RootDir+'search.dat','')
                except: pass
                return
            except:
