...
class MyWindowCountDownWithText2(xbmcgui.WindowDialog):
	scr={}; #scr['L']=0; scr['T']=0; scr['W']=1280; scr['H']=720; 
	def __init__(self,msg='',bgArt='',L=0,T=0,W=1280,H=720,TxtColor='0xFFFFFFFF',Font='font14',BorderWidth=10,ImgexitBtn='',colorDiffuse='0xff00ff00'):
		self.background=bgArt; self.scr['L']=L; self.scr['T']=T; self.scr['W']=W; self.scr['H']=H; 
		self.BG=xbmcgui.ControlImage(self.scr['L'],self.scr['T'],self.scr['W'],self.scr['H'],self.background,aspectRatio=0,colorDiffuse=colorDiffuse); 
		self.addControl(self.BG); 
		self.TxtMessage=xbmcgui.ControlTextBox(self.scr['L']+BorderWidth,self.scr['T']+BorderWidth,self.scr['W']-(BorderWidth*2),self.scr['H']-(BorderWidth*2),font=Font,textColor=TxtColor); 
		self.addControl(self.TxtMessage); 
...
def do_My_TextSplash2(msg='',img='http://xbmchub.com/images/index-logo.png',HowLong=10,resize=False,L=0,T=0,W=1280,H=720,TxtColor='0xFFFFFFFF',Font='font14',BorderWidth=10,ImgexitBtn='',colorDiffuse='0xff00ff00'): #HowLong in seconds.
	if resize==False: maxW=1280; maxH=720; W=maxW/2; H=maxH/2; L=maxW/4; T=maxH/4; 
	TempWindow2=MyWindowCountDownWithText2(msg=msg,bgArt=img,L=L,T=T,W=W,H=H,TxtColor=TxtColor,Font=Font,BorderWidth=BorderWidth,ImgexitBtn=ImgexitBtn,colorDiffuse=colorDiffuse); 
	#StartTime=time.clock(); 
	#while (time.clock()-StartTime) < HowLong:
	#	TempWindow2.show(); 
	TempWindow2.doModal(); 
